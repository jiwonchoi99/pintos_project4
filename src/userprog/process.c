...
bool handle_mm_fault(struct vm_entry *vme){
	...
	struct page *kpage = page_allocation(PAL_USER);
  ...
  switch (vme->page_type){
		case VM_BIN:
			...
			if (!install_page(vme->virtual_addr, kpage->kaddr, vme->can_write)){
				kaddr_page_free(kpage->kaddr);
				return false;
			}
			vme->load_flag = 1;
		...
		case VM_ANON:
      swap_in(kpage->kaddr, vme->swap_slot);
      install_page(vme->virtual_addr, kpage->kaddr, vme->can_write);
      vme->load_flag = 1;
      return true;
...
struct list_elem *execute_munmap(struct mmap_file * mmap_file){
	...
	for (e = list_begin(&mmap_file->vme_list); e!= list_end(&mmap_file->vme_list); ){
		...
		struct vm_entry *vme = list_entry(e, struct vm_entry, elem_mmap);
		if(vme->load_flag){
			...
			kaddr_page_free(kaddr);
			pagedir_clear_page(pagedir, vme->virtual_addr);
		}

		e = list_remove(e);
		vme_deletion(&thread_current()->vm, vme);

		free(vme);
	}
	struct list_elem *e_;
	e_ = list_remove(&mmap_file->elem);
...
bool expand_stack (void *addr){
  ...
  struct page *kpage = page_allocation(PAL_USER | PAL_ZERO);
  ...
  memset(vme, 0, sizeof(struct vm_entry));
  vme->virtual_addr = pg_round_down(addr);
  vme->page_type = VM_ANON;
  vme->load_flag = 1;
  vme->can_write = 1;
  vme_insertion(&thread_current()->vm, vme);
  ...
  success = install_page(vme->virtual_addr, kpage->kaddr, 1);
  if(!success){
    kaddr_page_free(kpage->kaddr);
  }
